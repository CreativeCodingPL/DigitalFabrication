...
void draw() {
  if (frameCount < 309) { //zapisujemy tylko pierwsze 309 klatek
    ...
    println("frame: " + frameCount + " locX: " + locX + " locY: " + locY+" rozmiar:" + rozmiar);
  } else {
    ...
    for (int i=0; i<309; i++) {
      ...
      rect(arrayX[i], arrayY[i], rad[i]/2, rad[i]*1.01);
    }
